...
feature "READ:" do
  ...
  scenario "INDEX: roles#index displays a list of rows", points: 1 do
    ...
    movie = FactoryGirl.create(:movie, director: director)
    first_role = FactoryGirl.create(:role, :character_name => "Superman", actor: actor, movie: movie)
    second_role = FactoryGirl.create(:role, :character_name => "Batman", actor:actor, movie: movie)
